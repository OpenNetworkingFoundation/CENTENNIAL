...
public class GetCapacity 
{
	...
	String capacityType;
	String LTPID;
	public GetCapacity(String nodeName,String LTPID)
	{
	...
	public void initialize()
	{
		...
		url = RestConfURL.CONTROLLER_URL.getUrl() + RestConfURL.NODE_URL.getUrl().replace("<NODE_ID_OR_NAME>", nodeName) + 
				RestConfURL.LP_URL.getUrl().replace("<LTP_ID>", LTPID).replace("<LP_ID>", LPID) + getCapability(LTPID);
	}
	...
	public String getPacType(String LTPID)
	{
    	...
    	{
			return "tdm-container-2-0:tdm-container-capability";
    	}else if(Repository.LogicalProtocolMap.get(LTPID).contains("ethernet-container"))
    	{
    		return "ethernet-container-2-0:ethernet-container-capability";
    	}else if(Repository.LogicalProtocolMap.get(LTPID).contains("pure-ethernet-structure"))
    	{
    		return "pure-ethernet-structure-2-0:pure-ethernet-structure-capability";
    	}else if(Repository.LogicalProtocolMap.get(LTPID).contains("hybrid-mw-structure"))
    	{
    		return "hybrid-mw-structure-2-0:hybrid-mw-structure-capability";
    	}else if(Repository.LogicalProtocolMap.get(LTPID).contains("wire-interface"))
    	{
    		return "wire-interface-2-0:wire-interface-capability";
    	}else if(Repository.LogicalProtocolMap.get(LTPID).contains("mac-interface"))
    	{
    		return "mac-interface-1-0:mac-interface-capability";
    	}else
    	{
    		return "air-interface-2-0:air-interface-capability";
    	}
		
	}
	
	public String getCapability(String LTPID)
	{
		if(Repository.LogicalProtocolMap.get(LTPID).contains("tdm-container"))
    	{
			return RestConfURL.TDM_CAPABILITY.getUrl();
    	}else if(Repository.LogicalProtocolMap.get(LTPID).contains("ethernet-container"))
    	{
    		return RestConfURL.ETHERNET_CAPABILITY.getUrl();
    	}else if(Repository.LogicalProtocolMap.get(LTPID).contains("pure-ethernet-structure"))
    	{
    		return RestConfURL.PURE_ETHERNET_CAPABILITY.getUrl();
    	}else if(Repository.LogicalProtocolMap.get(LTPID).contains("hybrid-mw-structure"))
    	{
    		return RestConfURL.HYBRID_MICROWAVE_STRUCTURE_CAPABILITY.getUrl();
    	}else if(Repository.LogicalProtocolMap.get(LTPID).contains("wire-interface"))
    	{
    		return RestConfURL.WIRE_INTERFACE_CAPABILITY.getUrl();
    	}else if(Repository.LogicalProtocolMap.get(LTPID).contains("mac-interface"))
    	{
    		return RestConfURL.MAC_INTERFACE_CAPABILITY.getUrl();
    	}else
    	{
    		return RestConfURL.AIR_INTERFACE_CAPABILITY.getUrl();
    	}
